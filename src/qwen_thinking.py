"""
Qwen 3.5 397B - Thinking ENABLED
===================================
Server must be started WITHOUT --chat-template-kwargs, or with:
  --chat-template-kwargs '{"enable_thinking":true}'
Output: qwen_thinking_predictions.jsonl
"""
...
OUT_PATH = Path("data/processed/qwen_thinking_predictions.jsonl")
...
def call_llm(system: str, user: str) -> dict:
    ...
    # With thinking ON, the answer is in "content" and thinking is in "reasoning_content"
    # If content is empty, the model ran out of tokens during thinking
    # In that case, try to find JSON in the reasoning_content
    if not text and reasoning:
        # Try to find JSON answer in the reasoning text
        json_match = re.search(r'\{[^{}]*"is_hazard"[^{}]*\}', reasoning)
        if json_match:
            text = json_match.group(0)
        else:
            # No JSON found anywhere — default to empty
            text = ""
...
def main():
    ...
    print("=" * 60)
    print("QWEN 3.5 397B — THINKING ENABLED")
    print("=" * 60)
    print("IMPORTANT: Ensure server was started WITHOUT the enable_thinking flag")
    print("  or with enable_thinking set to true")
    print()
    ...
    with OUT_PATH.open("a", encoding="utf-8") as f:
        for _, row in df.iterrows():
            ...
            record = {
                "id": pid,
                "paragraph": paragraph,
                "qwen_thinking_is_hazard": pred["is_hazard"],
                "qwen_thinking_condition": pred["condition"],
                "qwen_thinking_operation": pred["operation"],
                "qwen_thinking_state": pred["state"],
                "qwen_thinking_hazard_type": pred["hazard_type"],
            }
