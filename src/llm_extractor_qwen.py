...
OUT_PATH = Path("data/processed/qwen_predictions.jsonl")

# ---------- Qwen via llama.cpp config ----------
# Make sure SSH tunnel is active: ssh -L 8080:localhost:8080 username@workstation-ip
...
# Only used to catch PURELY definitional/explanatory paragraphs
PURE_BENIGN_MARKERS = [
    "is defined as",
    "refers to",
    "for the purposes of",
...
# ── Prefilter: WIDE net, only block the clearly benign ──────────────────────
def prefilter_maybe_hazard(paragraph: str) -> bool:
    t = paragraph.lower()

    # ── Hard block 1: message table headers ──
    if t.startswith("this message is sent by"):
        return False

    # ── Hard block 2: state/purpose descriptions ──
    ...
    # ── Hard block 3: pure definitional paragraphs ──
    if any(b in t for b in PURE_BENIGN_MARKERS):
        has_any_risky = any(k in t for k in STRONG_RISKY + MEDIUM_RISKY)
        if not has_any_risky:
            return False
...
# ── Post-filter: validate that extracted operation is genuinely risky ─────────
def postfilter_valid_extraction(operation: str, paragraph: str) -> bool:
    op_lower = operation.lower()
    para_lower = paragraph.lower()

    # Operation must contain a risky verb OR paragraph clearly does
    ...
    # Reject if operation is ONLY "send a/the reject" with no destructive verb
...
# ── LLM call (llama.cpp OpenAI-compatible API) ───────────────────────────────
def call_llm(system: str, user: str) -> dict:
    ...
    # Qwen3.5 with thinking=1 may wrap response in <think>...</think> tags
    # Strip the thinking portion and keep only the final answer
    import re
...
def main():
    ...
    with OUT_PATH.open("a", encoding="utf-8") as f:
        for _, row in df.iterrows():
            ...
            # ── Stage 0: Prefilter (broad — only blocks clearly benign) ──
            ...
            record = {
                "id": pid,
                "paragraph": paragraph,
                "qwen_is_hazard": pred["is_hazard"],
                "qwen_condition": pred["condition"],
                "qwen_operation": pred["operation"],
                "qwen_state": pred["state"],
                "qwen_hazard_type": pred["hazard_type"],
            }
    ...
    print(f"Predictions appended to {OUT_PATH}")
